...
/*  Notes on UART4_tx_ISR()

    This function is an Interrupt Vector Table compatible ISR to respond to the
    U4TX interrupt signal. This signal is generated whenever U4TXB is empty and
    U4TXIE is enabled.
*/

// wrappers to make the register accesses easier
#define UART4_TX_IE_enable() U4TXIE = 1
#define UART4_TX_IE_disable() U4TXIE = 0

void __interrupt(irq(U4TX), high_priority) UART4_tx_ISR() {
    if (tx_buffer_is_empty()) {
        UART4_TX_IE_disable();
    } else {
        U4TXB = tx_buffer_read();
    }
}

/*  Notes on using UART4_tx_string()

    The following block handles the case when this function overflows the
    UART4_tx_buffer. There are three options for dealing with this input
    overflow:
...
*/
void UART4_tx_string(const char *string, const char terminator) {
...
/*  Notes on UART4_rx_ISR()

    This function is an Interrupt Vector Table compatible ISR to respond to the
    U4RX interrupt signal. This signal is generated whenever there is an unread
    byte in U4RXB.
*/

void __interrupt(irq(U4RX), high_priority) UART4_rx_ISR() {
    rx_buffer_write(U4RXB);
}

char UART4_rx_char(void) {
    char data = 0;
...
uint8_t UART4_rx_available(void) {
    if (rx_buffer_is_empty()) {
        return 0;
    }
...
static uart_interface_t UART4_create(uart_config_t config) {
    ...
    interface.tx_string = UART4_tx_string;
...
static void UART4_baud_select(baud_rate_t baudRate) {
    U4BRG = baudTable[baudRate];
}

static void UART4_pps_init(uart_config_t config) {
    // Only set the PPS pins if the user has actually supplied a value
    if (config.txPin) {
        pps_out_UART4_TX(config.txPin);
    }
    if (config.rxPin) {
        pps_in_UART4_RX(config.rxPin);
    }
}

uart_interface_t UART4_init(uart_config_t config) {
    UART4_baud_select(config.baud);
    UART4_pps_init(config);
    ...
    U4CON1bits.ON = 1; // Enable UART4

    return UART4_create(config);
}